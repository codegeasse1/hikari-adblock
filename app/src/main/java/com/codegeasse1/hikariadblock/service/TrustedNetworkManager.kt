 ...
 * enabled, auto-pauses Hikari AdBlock on a trusted SSID and auto-resumes when the
 * device leaves it. Registered once from [HikariApp] so it runs for
...
class TrustedNetworkManager(
            ...
            lock.withLock {
                try {
                    ...
                    when {
                        ...
                        onTrusted && running -> {
                            Timber.d("Trusted network '$ssid' — pausing Hikari AdBlock")
                        ...
                        !onTrusted && pausedByUs && !running -> {
                            Timber.d("Left trusted network (now '$ssid') — resuming Hikari AdBlock")
                            appPrefs.setPausedByTrusted(false)
                            cancelPausedNotification()
                            ServiceController.requestStart(context)
                        }
    ...
    private fun showPausedNotification(ssid: String) {
        try {
            ...
            val tapIntent = PendingIntent.getActivity(
                context, 0,
                android.content.Intent(context, com.codegeasse1.hikariadblock.MainActivity::class.java),
    ...
    companion object {
        private const val CHANNEL_ID = "hikari_trusted_channel"
